...
void BrownianThermostat::apply (std::vector<BaseParticle *> &particles, llint curr_step) {
	...
	for(auto p: particles) {
		if(drand48() < _pt) {
			p->vel = LR_vector(Utils::gaussian(), Utils::gaussian(), Utils::gaussian()) * _rescale_factor; //subho *sqrt(p->massinverted) for variable mass to work
		}
		if(drand48() < _pr) {
			p->L = LR_vector(Utils::gaussian(), Utils::gaussian(), Utils::gaussian()) * _rescale_factor; //subho
		}
	}
}
