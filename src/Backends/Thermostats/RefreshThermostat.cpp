...
void RefreshThermostat::apply(std::vector<BaseParticle *> &particles, llint curr_step) {
	...
	for(auto p: particles) {
		p->vel = LR_vector(Utils::gaussian(), Utils::gaussian(), Utils::gaussian()); //subho *sqrt(p->massinverted)
		p->L = LR_vector(Utils::gaussian(), Utils::gaussian(), Utils::gaussian()); //subho
	}
}
