...
__forceinline__ __device__ void gaussian(hiprandState &state, float &outx, float &outy) {
	float r = sqrtf(-2.0f * logf(hiprand_uniform(&state)));
	float phi = 2.f * PI * hiprand_uniform(&state);

	outx = r * __cosf(phi);
	outy = r * __sinf(phi);
}

__forceinline__ __device__ void gaussian(hiprandState &state, double &outx, double &outy) {
	double r = sqrt(-2. * log(hiprand_uniform(&state)));
	double phi = 2 * M_PI * hiprand_uniform(&state);
