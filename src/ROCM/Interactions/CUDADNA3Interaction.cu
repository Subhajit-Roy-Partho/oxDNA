...
CUDADNA3Interaction::~CUDADNA3Interaction() {
    if(_d_is_strand_end != nullptr) {
        CUDA_SAFE_CALL(hipFree(_d_is_strand_end));
    }
    if(_d_particle_types != nullptr) {
        CUDA_SAFE_CALL(hipFree(_d_particle_types));
    }
}
...
void CUDADNA3Interaction::cuda_init(int N) {
    ...
    CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(MD_fene_r0_SD), params.data(), params.size() * sizeof(float)));
    params = _convert_param_array(&this->_fene_delta2_SD, 1);
    CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(MD_fene_delta2_SD), params.data(), params.size() * sizeof(float)));
    params = _convert_param_array(&this->_mbf_xmax_SD, 1);
    CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(MD_mbf_xmax_SD), params.data(), params.size() * sizeof(float)));
    COPY_NUMBER_TO_FLOAT(MD_fene_eps, this->_fene_eps);

    // excluded volume
    params = _convert_param_array(this->_excl_s, 7);
    CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(MD_excl_s), params.data(), params.size() * sizeof(float)));
    params = _convert_param_array(this->_excl_r, 7);
    CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(MD_excl_r), params.data(), params.size() * sizeof(float)));
    params = _convert_param_array(this->_excl_b, 7);
    CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(MD_excl_b), params.data(), params.size() * sizeof(float)));
    params = _convert_param_array(this->_excl_rc, 7);
    CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(MD_excl_rc), params.data(), params.size() * sizeof(float)));

    params = _convert_param_array(F1_SD_EPS, 2);
    CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(MD_F1_SD_EPS), params.data(), params.size() * sizeof(float)));
    params = _convert_param_array(F1_SD_A, 2);
    CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(MD_F1_SD_A), params.data(), params.size() * sizeof(float)));
    params = _convert_param_array(F1_SD_RC, 2);
    CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(MD_F1_SD_RC), params.data(), params.size() * sizeof(float)));
    params = _convert_param_array(F1_SD_R0, 2);
    CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(MD_F1_SD_R0), params.data(), params.size() * sizeof(float)));
    params = _convert_param_array(F1_SD_BLOW, 2);
    CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(MD_F1_SD_BLOW), params.data(), params.size() * sizeof(float)));
    params = _convert_param_array(F1_SD_BHIGH, 2);
    CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(MD_F1_SD_BHIGH), params.data(), params.size() * sizeof(float)));
    params = _convert_param_array(F1_SD_RLOW, 2);
    CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(MD_F1_SD_RLOW), params.data(), params.size() * sizeof(float)));
    params = _convert_param_array(F1_SD_RHIGH, 2);
    CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(MD_F1_SD_RHIGH), params.data(), params.size() * sizeof(float)));
    params = _convert_param_array(F1_SD_RCLOW, 2);
    CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(MD_F1_SD_RCLOW), params.data(), params.size() * sizeof(float)));
    params = _convert_param_array(F1_SD_RCHIGH, 2);
    CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(MD_F1_SD_RCHIGH), params.data(), params.size() * sizeof(float)));
    params = _convert_param_array(F1_SD_SHIFT, 2);
    CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(MD_F1_SD_SHIFT), params.data(), params.size() * sizeof(float)));

    params = _convert_param_array(F2_SD_K, 4);
    CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(MD_F2_SD_K), params.data(), params.size() * sizeof(float)));
    params = _convert_param_array(F2_SD_RC, 4);
    CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(MD_F2_SD_RC), params.data(), params.size() * sizeof(float)));
    params = _convert_param_array(F2_SD_R0, 4);
    CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(MD_F2_SD_R0), params.data(), params.size() * sizeof(float)));
    params = _convert_param_array(F2_SD_BLOW, 4);
    CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(MD_F2_SD_BLOW), params.data(), params.size() * sizeof(float)));
    params = _convert_param_array(F2_SD_RLOW, 4);
    CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(MD_F2_SD_RLOW), params.data(), params.size() * sizeof(float)));
    params = _convert_param_array(F2_SD_RCLOW, 4);
    CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(MD_F2_SD_RCLOW), params.data(), params.size() * sizeof(float)));
    params = _convert_param_array(F2_SD_BHIGH, 4);
    CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(MD_F2_SD_BHIGH), params.data(), params.size() * sizeof(float)));
    params = _convert_param_array(F2_SD_RCHIGH, 4);
    CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(MD_F2_SD_RCHIGH), params.data(), params.size() * sizeof(float)));
    params = _convert_param_array(F2_SD_RHIGH, 4);
    CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(MD_F2_SD_RHIGH), params.data(), params.size() * sizeof(float)));

    params = _convert_param_array(F4_SD_THETA_A, 21);
    CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(MD_F4_SD_THETA_A), params.data(), params.size() * sizeof(float)));
    params = _convert_param_array(F4_SD_THETA_B, 21);
    CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(MD_F4_SD_THETA_B), params.data(), params.size() * sizeof(float)));
    params = _convert_param_array(F4_SD_THETA_T0, 21);
    CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(MD_F4_SD_THETA_T0), params.data(), params.size() * sizeof(float)));
    params = _convert_param_array(F4_SD_THETA_TS, 21);
    CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(MD_F4_SD_THETA_TS), params.data(), params.size() * sizeof(float)));
    params = _convert_param_array(F4_SD_THETA_TC, 21);
    CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(MD_F4_SD_THETA_TC), params.data(), params.size() * sizeof(float)));

    params = _convert_param_array(F5_SD_PHI_A, 4);
    CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(MD_F5_SD_PHI_A), params.data(), params.size() * sizeof(float)));
    params = _convert_param_array(F5_SD_PHI_B, 4);
    CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(MD_F5_SD_PHI_B), params.data(), params.size() * sizeof(float)));
    params = _convert_param_array(F5_SD_PHI_XC, 4);
    CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(MD_F5_SD_PHI_XC), params.data(), params.size() * sizeof(float)));
    params = _convert_param_array(F5_SD_PHI_XS, 4);
    CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(MD_F5_SD_PHI_XS), params.data(), params.size() * sizeof(float)));
    ...
    CUDA_SAFE_CALL(hipMemcpy(_d_particle_types, particle_types.data(), sizeof(uint8_t) * _N, hipMemcpyHostToDevice));
...
void CUDADNA3Interaction::compute_forces(CUDABaseList *lists, c_number4 *d_poss, GPU_quat *d_orientations, c_number4 *d_forces, c_number4 *d_torques, LR_bonds *d_bonds, CUDABox*d_box) {
    ...
    if(_update_st) {
        CUDA_SAFE_CALL(hipMemset(_d_st, 0, _N * sizeof(CUDAStressTensor)));
    }
