...
CUDATEPInteraction::~CUDATEPInteraction() {
	if(_d_kb1_pref != NULL) CUDA_SAFE_CALL(hipFree(_d_kb1_pref));
	if(_d_kb2_pref != NULL) CUDA_SAFE_CALL(hipFree(_d_kb2_pref));
	if(_d_xk_bending != NULL) CUDA_SAFE_CALL(hipFree(_d_xk_bending));
	if(_d_xu_bending != NULL) CUDA_SAFE_CALL(hipFree(_d_xu_bending));
	if(_d_kt_pref != NULL) CUDA_SAFE_CALL(hipFree(_d_kt_pref));
	if(_d_o_vects != NULL) CUDA_SAFE_CALL(hipFree(_d_o_vects));
	if(_d_w_vects != NULL) CUDA_SAFE_CALL(hipFree(_d_w_vects));
}
...
void CUDATEPInteraction::cuda_init(int N) {
	...
	CUDA_SAFE_CALL(hipMemcpy(_d_kb1_pref, this->_kb1_pref, k_size, hipMemcpyHostToDevice));
	CUDA_SAFE_CALL(hipMemcpy(_d_kb2_pref, this->_kb2_pref, k_size, hipMemcpyHostToDevice));
	CUDA_SAFE_CALL(hipMemcpy(_d_xk_bending, this->_xk_bending, k_size, hipMemcpyHostToDevice));
	CUDA_SAFE_CALL(hipMemcpy(_d_xu_bending, this->_xu_bending, k_size, hipMemcpyHostToDevice));
	CUDA_SAFE_CALL(hipMemcpy(_d_kt_pref, this->_kt_pref, k_size, hipMemcpyHostToDevice));
	...
	CUDA_SAFE_CALL(hipMemcpy(_d_o_vects, h_o_vects, 2 * sizeof(c_number4), hipMemcpyHostToDevice));
	CUDA_SAFE_CALL(hipMemcpy(_d_w_vects, h_w_vects, 2 * sizeof(c_number4), hipMemcpyHostToDevice));
	...
	CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(MD_o_modulus), o_modulus, 2 * sizeof(float)));
