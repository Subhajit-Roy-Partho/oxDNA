...
CUDADNANMInteraction::~CUDADNANMInteraction() {
    ...
    if(_d_ang_kbkt != NULL) CUDA_SAFE_CALL( hipFree(_d_ang_kbkt) );
...
//    Addition of Reading Parameter File -> Moved from get_settings due to needing to fill variables that are filled in the CPU version of DNANMInteraction::read_topology
    ...
    if(_read_par){
        ...
        if(_angular) { // only need for anmt
            ...
            if (this->_parameter_kbkt) {
                ...
                CUDA_SAFE_CALL(hipMalloc(&_d_ang_kbkt, sizeof(c_number) * (this->npro * 2)));
            }
        }
        ...
        auto valid_spring_params = [](int num_particles, int x, int y, double d, char s, double k){
            if(x < 0 || x > num_particles) throw oxDNAException("Invalid Particle ID %d in Parameter File", x);
            if(y < 0 || y > num_particles) throw oxDNAException("Invalid Particle ID %d in Parameter File", y);
        ...
        if(_angular) {
            ...
            if (this->_parameter_kbkt) {
                CUDA_SAFE_CALL(hipMemcpy(_d_ang_kbkt, _h_ang_kbkt, sizeof(c_number) * (this->npro * 2),
    ...
    CUDA_SAFE_CALL( hipMemcpyToSymbol(HIP_SYMBOL(MD_pro_sqr_rcut), &_pro_sqr_rcut, sizeof(float)) );
    ...
    CUDA_SAFE_CALL( hipMemcpyToSymbol(HIP_SYMBOL(MD_pro_backbone_sqr_rcut), &_pro_backbone_sqr_rcut, sizeof(float)) );
    ...
    CUDA_SAFE_CALL( hipMemcpyToSymbol(HIP_SYMBOL(MD_pro_base_sqr_rcut), &_pro_base_sqr_rcut, sizeof(float)) );

    CUDA_SAFE_CALL( hipMemcpyToSymbol(HIP_SYMBOL(MD_dna_sqr_rcut), &this->_sqr_rcut, sizeof(float)) );

    //Parameters for DNANM book keeping
    CUDA_SAFE_CALL( hipMemcpyToSymbol(HIP_SYMBOL(_ndna), &this->ndna, sizeof(int)) );
