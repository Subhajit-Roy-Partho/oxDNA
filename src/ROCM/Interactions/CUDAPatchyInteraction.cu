...
void CUDAPatchyInteraction::cuda_init(int N) {
	...
	if(this->_is_binary) CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(MD_N_patches), &this->_N_patches_B, sizeof(int), sizeof(int)));
	...
	for(int i = 0; i < limit; i++) {
		...
		CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(MD_base_patches), base_patches, sizeof(float4)*n_patches, i*sizeof(float4)*CUDA_MAX_PATCHES));
