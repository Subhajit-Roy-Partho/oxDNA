...
CUDABaseInteraction::~CUDABaseInteraction() {
	if(_use_edge) {
		if(_d_edge_forces != NULL) CUDA_SAFE_CALL(hipFree(_d_edge_forces));
		if(_d_edge_torques != NULL) CUDA_SAFE_CALL(hipFree(_d_edge_torques));
	}

	if(_d_st != nullptr) {
		CUDA_SAFE_CALL(hipFree(_d_st));
	}
}
...
void CUDABaseInteraction::get_cuda_settings(input_file &inp) {
	...
	int update_state = getInputInt(&inp, "ROCM_update_stress_tensor_every", &update_st_every, 0);
	if(update_state == KEY_NOT_FOUND) {
...
void CUDABaseInteraction::cuda_init(int N) {
	...
	if(_use_edge && _d_edge_forces == nullptr) {
		...
		CUDA_SAFE_CALL(hipMemset(_d_edge_forces, 0, size));
		CUDA_SAFE_CALL(hipMemset(_d_edge_torques, 0, size));
	}
...
StressTensor CUDABaseInteraction::CPU_stress_tensor(c_number4 *vels) {
	std::vector<CUDAStressTensor> host_st(_N);
	std::vector<c_number4> host_vels(_N);
	CUDA_SAFE_CALL(hipMemcpy(host_st.data(), _d_st, sizeof(CUDAStressTensor) * _N, hipMemcpyDeviceToHost));
	CUDA_SAFE_CALL(hipMemcpy(host_vels.data(), vels, sizeof(c_number4) * _N, hipMemcpyDeviceToHost));

	CUDAStressTensor st_sum;
	for(const auto &tensor : host_st) {
		st_sum += tensor;
	}
	for(const auto &velocity : host_vels) {
		st_sum += CUDAStressTensor(
			SQR(velocity.x),
			SQR(velocity.y),
			SQR(velocity.z),
