...
void CUDALJInteraction::cuda_init(int N) {
	...
	CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(MD_LJ_n), &this->_n, 3 * sizeof(int)));
	...
	if(this->_use_edge) {
		CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(MD_n_forces), &this->_n_forces, sizeof(int)));
	}
}
