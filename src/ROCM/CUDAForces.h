...
void init_COMForce_from_CPU(COM_force *cuda_force, COMForce *cpu_force, bool first_time) {
	...
	CUDA_SAFE_CALL(hipMemcpy(cuda_force->com_indexes, local_com_indexes.data(), sizeof(int) * local_com_indexes.size(), hipMemcpyHostToDevice));
	CUDA_SAFE_CALL(hipMemcpy(cuda_force->ref_indexes, local_ref_indexes.data(), sizeof(int) * local_ref_indexes.size(), hipMemcpyHostToDevice));
}
...
void init_LTCOMTrap_from_CPU(lt_com_trap *cuda_force, LTCOMTrap *cpu_force, bool first_time) {
	...
	CUDA_SAFE_CALL(hipMemcpy(cuda_force->p1a, local_p1a.data(), sizeof(int) * local_p1a.size(), hipMemcpyHostToDevice));
	CUDA_SAFE_CALL(hipMemcpy(cuda_force->p2a, local_p2a.data(), sizeof(int) * local_p2a.size(), hipMemcpyHostToDevice));
	CUDA_SAFE_CALL(hipMemcpy(cuda_force->potential_grid, local_grid.data(), sizeof(c_number) * local_grid.size(), hipMemcpyHostToDevice));
}
