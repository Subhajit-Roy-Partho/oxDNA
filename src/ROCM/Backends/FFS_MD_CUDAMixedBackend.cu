...
FFS_MD_CUDAMixedBackend::~FFS_MD_CUDAMixedBackend() {
	...
	CUDA_SAFE_CALL(hipFree(_d_hb_pairs1));
	CUDA_SAFE_CALL(hipFree(_d_hb_pairs2));
	CUDA_SAFE_CALL(hipFree(_d_dist_pairs1));
	CUDA_SAFE_CALL(hipFree(_d_dist_pairs2));
	CUDA_SAFE_CALL(hipFree(_d_op_dists));
	CUDA_SAFE_CALL(hipFree(_d_hb_energies));
	CUDA_SAFE_CALL(hipFree(_d_dist_region_lens));
	CUDA_SAFE_CALL(hipFree(_d_dist_region_rows));
	CUDA_SAFE_CALL(hipFree(_d_hb_region_lens));
	CUDA_SAFE_CALL(hipFree(_d_hb_region_rows));
	CUDA_SAFE_CALL(hipFree(_d_hb_cutoffs));
	CUDA_SAFE_CALL(hipFree(_d_region_is_nearhb));
...
void FFS_MD_CUDAMixedBackend::_free_simple_conditions(SimpleConditions sc) {
	...
	free(sc.h_ffs_stop);
	CUDA_SAFE_CALL(hipFree(sc.d_dist_cond_lens));
	CUDA_SAFE_CALL(hipFree(sc.d_dist_cond_rows));
	CUDA_SAFE_CALL(hipFree(sc.d_hb_cond_lens));
	CUDA_SAFE_CALL(hipFree(sc.d_hb_cond_rows));
	CUDA_SAFE_CALL(hipFree(sc.d_hb_cond_mags));
	CUDA_SAFE_CALL(hipFree(sc.d_hb_cond_types));
	CUDA_SAFE_CALL(hipFree(sc.d_nearhb_cond_lens));
	CUDA_SAFE_CALL(hipFree(sc.d_nearhb_cond_rows));
	CUDA_SAFE_CALL(hipFree(sc.d_nearhb_cond_mags));
	CUDA_SAFE_CALL(hipFree(sc.d_nearhb_cond_types));
	CUDA_SAFE_CALL(hipFree(sc.d_dist_cond_mags));
	CUDA_SAFE_CALL(hipFree(sc.d_dist_cond_types));
	CUDA_SAFE_CALL(hipFree(sc.d_ffs_stop));
}
...
void FFS_MD_CUDAMixedBackend::init() {
	...
	CUDA_SAFE_CALL(hipMemcpy(_d_dist_region_lens, h_dist_region_lens, _n_dist_regions * sizeof(int), hipMemcpyHostToDevice));
	CUDA_SAFE_CALL(hipMemcpy(_d_dist_region_rows, h_dist_region_rows, _n_dist_regions * sizeof(int), hipMemcpyHostToDevice));
	CUDA_SAFE_CALL(hipMemcpy(_d_hb_region_lens, h_hb_region_lens, _n_hb_regions * sizeof(int), hipMemcpyHostToDevice));
	CUDA_SAFE_CALL(hipMemcpy(_d_hb_region_rows, h_hb_region_rows, _n_hb_regions * sizeof(int), hipMemcpyHostToDevice));

	CUDA_SAFE_CALL(hipMemcpy(_d_hb_pairs1, h_hb_pairs1, _n_hb_pairs * sizeof(int), hipMemcpyHostToDevice));
	CUDA_SAFE_CALL(hipMemcpy(_d_hb_pairs2, h_hb_pairs2, _n_hb_pairs * sizeof(int), hipMemcpyHostToDevice));
	CUDA_SAFE_CALL(hipMemcpy(_d_dist_pairs1, h_dist_pairs1, _n_dist_pairs * sizeof(int), hipMemcpyHostToDevice));
	CUDA_SAFE_CALL(hipMemcpy(_d_dist_pairs2, h_dist_pairs2, _n_dist_pairs * sizeof(int), hipMemcpyHostToDevice));
	CUDA_SAFE_CALL(hipMemcpy(_d_hb_cutoffs, h_hb_cutoffs, _n_hb_regions * sizeof(float), hipMemcpyHostToDevice));
	CUDA_SAFE_CALL(hipMemcpy(_d_region_is_nearhb, _h_region_is_nearhb, _n_hb_regions * sizeof(bool), hipMemcpyHostToDevice));
...
SimpleConditions FFS_MD_CUDAMixedBackend::_get_simple_conditions(std::vector<parsed_condition> conditions, const char type[256]) {
	...
	CUDA_SAFE_CALL(hipMemcpy(sc.d_hb_cond_mags, sc.h_hb_cond_mags, sc.hb_cond_len * sizeof(float), hipMemcpyHostToDevice));
	CUDA_SAFE_CALL(hipMemcpy(sc.d_hb_cond_types, sc.h_hb_cond_types, sc.hb_cond_len * sizeof(int), hipMemcpyHostToDevice));
	CUDA_SAFE_CALL(hipMemcpy(sc.d_hb_cond_lens, sc.h_hb_cond_lens, _n_hb_regions * sizeof(int), hipMemcpyHostToDevice));
	CUDA_SAFE_CALL(hipMemcpy(sc.d_hb_cond_rows, sc.h_hb_cond_rows, _n_hb_regions * sizeof(int), hipMemcpyHostToDevice));
	CUDA_SAFE_CALL(hipMemcpy(sc.d_nearhb_cond_mags, sc.h_nearhb_cond_mags, sc.nearhb_cond_len * sizeof(float), hipMemcpyHostToDevice));
	CUDA_SAFE_CALL(hipMemcpy(sc.d_nearhb_cond_types, sc.h_nearhb_cond_types, sc.nearhb_cond_len * sizeof(int), hipMemcpyHostToDevice));
	CUDA_SAFE_CALL(hipMemcpy(sc.d_nearhb_cond_lens, sc.h_nearhb_cond_lens, _n_hb_regions * sizeof(int), hipMemcpyHostToDevice));
	CUDA_SAFE_CALL(hipMemcpy(sc.d_nearhb_cond_rows, sc.h_nearhb_cond_rows, _n_hb_regions * sizeof(int), hipMemcpyHostToDevice));
	CUDA_SAFE_CALL(hipMemcpy(sc.d_dist_cond_mags, sc.h_dist_cond_mags, sc.dist_cond_len * sizeof(float), hipMemcpyHostToDevice));
	CUDA_SAFE_CALL(hipMemcpy(sc.d_dist_cond_types, sc.h_dist_cond_types, sc.dist_cond_len * sizeof(int), hipMemcpyHostToDevice));
	CUDA_SAFE_CALL(hipMemcpy(sc.d_dist_cond_lens, sc.h_dist_cond_lens, _n_dist_regions * sizeof(int), hipMemcpyHostToDevice));
	CUDA_SAFE_CALL(hipMemcpy(sc.d_dist_cond_rows, sc.h_dist_cond_rows, _n_dist_regions * sizeof(int), hipMemcpyHostToDevice));
	...
	while(getInputString(&input, pairname, strexpr, 0) == KEY_FOUND) {
		...
		if(strcmp(condition_set_type, "master") == 0) {
			...
			for(std::vector<parsed_expression>::iterator expression = newcondition.all_expressions.begin(); expression != newcondition.all_expressions.end(); expression++) {
				if((*expression).compare_type == -1) throw oxDNAException("Failed to parse condition %s, please check the file format and parameter names ", pairname);
			}
		}
		else {
...
void FFS_MD_CUDAMixedBackend::_eval_stop_conditions(SimpleConditions sc) {
	...
	hb_op_eval
		<<<_ffs_hb_eval_kernel_cfg.blocks, _ffs_hb_eval_kernel_cfg.threads_per_block>>>
		(_d_hb_energies, _d_hb_region_lens, _d_hb_region_rows, sc.d_hb_cond_lens, sc.d_hb_cond_rows, sc.d_hb_cond_mags, sc.d_hb_cond_types, sc.d_ffs_stop, _n_hb_regions, _d_hb_cutoffs);
	...
	nearhb_op_eval
		<<<_ffs_hb_eval_kernel_cfg.blocks, _ffs_hb_eval_kernel_cfg.threads_per_block>>>
		(_d_nearhb_states, _d_hb_region_lens, _d_hb_region_rows, sc.d_nearhb_cond_lens, sc.d_nearhb_cond_rows, sc.d_nearhb_cond_mags, sc.d_nearhb_cond_types, sc.d_ffs_stop, _n_hb_regions, sc.hb_cond_len + sc.dist_cond_len);
	...
	CUDA_SAFE_CALL(hipMemcpy(sc.h_ffs_stop, sc.d_ffs_stop, sc.stop_length * sizeof(bool), hipMemcpyDeviceToHost));
}
...
char * FFS_MD_CUDAMixedBackend::get_op_state_str(void) {
	CUDA_SAFE_CALL(hipMemcpy(_h_hb_energies, _d_hb_energies, _n_hb_pairs * sizeof(float), hipMemcpyDeviceToHost));
	CUDA_SAFE_CALL(hipMemcpy(_h_op_dists, _d_op_dists, _n_dist_pairs * sizeof(float), hipMemcpyDeviceToHost));
	CUDA_SAFE_CALL(hipMemcpy(_h_nearhb_states, _d_nearhb_states, _n_hb_pairs * sizeof(bool), hipMemcpyDeviceToHost));
	...
	for(int i = 0; i < _n_dist_regions; i++) {
		...
		for(int j = 1; j < h_dist_region_lens[i]; j++) {
			if(_h_op_dists[h_dist_region_rows[i] + j] < min_dist) {
