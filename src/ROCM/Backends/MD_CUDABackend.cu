...
MD_CUDABackend::~MD_CUDABackend() {
	if(_d_particles_to_mols != nullptr) {
		CUDA_SAFE_CALL(hipFree(_d_particles_to_mols));
	}

	if(_d_vels != nullptr) {
		CUDA_SAFE_CALL(hipFree(_d_vels));
		CUDA_SAFE_CALL(hipFree(_d_Ls));
		CUDA_SAFE_CALL(hipFree(_d_forces));
		CUDA_SAFE_CALL(hipFree(_d_torques));
	}

	if(_d_molecular_coms != nullptr) {
		CUDA_SAFE_CALL(hipFree(_d_molecular_coms));
	}

	if(_sort_every > 0 && _d_buff_vels != nullptr) {
		CUDA_SAFE_CALL(hipFree(_d_buff_vels));
		CUDA_SAFE_CALL(hipFree(_d_buff_Ls));
		CUDA_SAFE_CALL(hipFree(_d_buff_particles_to_mols));
	}
	...
	if(_external_forces) {
		if(_d_ext_forces != nullptr) {
			CUDA_SAFE_CALL(hipFree(_d_ext_forces));
		}
	}
...
void MD_CUDABackend::_host_to_gpu() {
	...
	CUDA_SAFE_CALL(hipMemcpy(_d_particles_to_mols, _h_particles_to_mols.data(), sizeof(int) * N(), hipMemcpyHostToDevice));
	CUDA_SAFE_CALL(hipMemcpy(_d_vels, _h_vels, _vec_size, hipMemcpyHostToDevice));
	CUDA_SAFE_CALL(hipMemcpy(_d_Ls, _h_Ls, _vec_size, hipMemcpyHostToDevice));
}

void MD_CUDABackend::_gpu_to_host() {
	CUDABaseBackend::_gpu_to_host();
	CUDA_SAFE_CALL(hipMemcpy(_h_particles_to_mols.data(), _d_particles_to_mols, sizeof(int) * N(), hipMemcpyDeviceToHost));
	CUDA_SAFE_CALL(hipMemcpy(_h_vels, _d_vels, _vec_size, hipMemcpyDeviceToHost));
	CUDA_SAFE_CALL(hipMemcpy(_h_Ls, _d_Ls, _vec_size, hipMemcpyDeviceToHost));
	CUDA_SAFE_CALL(hipMemcpy(_h_forces, _d_forces, _vec_size, hipMemcpyDeviceToHost));
	CUDA_SAFE_CALL(hipMemcpy(_h_torques, _d_torques, _vec_size, hipMemcpyDeviceToHost));
}
...
void MD_CUDABackend::_apply_external_forces_changes() {
	if(_external_forces) {
		...
		CUDA_SAFE_CALL(hipMemcpy(_d_ext_forces, h_ext_forces.data(), N() * MAX_EXT_FORCES * sizeof(CUDA_trap), hipMemcpyHostToDevice));
	}
}
...
void MD_CUDABackend::_init_CUDA_MD_symbols() {
	...
	CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(MD_sqr_verlet_skin), &f_copy, sizeof(float)));
	f_copy = _dt;
	CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(MD_dt), &f_copy, sizeof(float)));
	int myN = N();
	CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(MD_N), &myN, sizeof(int)));
}
...
void MD_CUDABackend::_rescale_molecular_positions(c_number4 new_Ls, c_number4 old_Ls, bool recompute_coms) {
	...
	if(recompute_coms) {
		CUDA_SAFE_CALL(hipMemset(_d_molecular_coms, 0, sizeof(c_number4) * _molecules.size()));
...
void MD_CUDABackend::_sort_particles() {
	...
	CUDA_SAFE_CALL(hipMemcpy(_d_orientations, _d_buff_orientations, _orient_size, hipMemcpyDeviceToDevice));

	// copy back the sorted vectors
	CUDA_SAFE_CALL(hipMemcpy(_d_poss, _d_buff_poss, _vec_size, hipMemcpyDeviceToDevice));
	CUDA_SAFE_CALL(hipMemcpy(_d_bonds, _d_buff_bonds, _bonds_size, hipMemcpyDeviceToDevice));
	CUDA_SAFE_CALL(hipMemcpy(_d_vels, _d_buff_vels, _vec_size, hipMemcpyDeviceToDevice));
	CUDA_SAFE_CALL(hipMemcpy(_d_Ls, _d_buff_Ls, _vec_size, hipMemcpyDeviceToDevice));
	CUDA_SAFE_CALL(hipMemcpy(_d_particles_to_mols, _d_buff_particles_to_mols, sizeof(int) * N(), hipMemcpyDeviceToDevice));
}
...
void MD_CUDABackend::init() {
	...
	CUDA_SAFE_CALL(hipMemset(_d_forces, 0, _vec_size));
	CUDA_SAFE_CALL(hipMemset(_d_torques, 0, _vec_size));
	...
	CUDA_SAFE_CALL(hipMemcpy(_d_mol_sizes, mol_sizes.data(), sizeof(int) * _molecules.size(), hipMemcpyHostToDevice));
