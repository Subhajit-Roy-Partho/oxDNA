...
__global__ void SRD_fill_cells_and_refresh(c_number4 *poss, c_number4 *vels, int *cells, int *counters_cells, c_number4 *cells_dp, bool *cell_overflow, hiprandState *rand_state, c_number rescale_factor) {
...
__global__ void SRD_init_particles(c_number4 *poss, c_number4 *vels, hiprandState *rand_state, c_number rescale_factor) {
	if(IND >= N_solvent[0]) return;

	hiprandState state = rand_state[IND];

	// randomly place the particles in the box
	c_number4 r = { hiprand_uniform(&state) * box_side[0], hiprand_uniform(&state) * box_side[0], hiprand_uniform(&state) * box_side[0], 0.f };
	poss[IND] = r;
