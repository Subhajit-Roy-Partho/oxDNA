...
std::shared_ptr<CUDABaseThermostat> CUDAThermostatFactory::make_thermostat(input_file &inp, BaseBox * box) {
	...
	else if(!strncmp(thermostat_type, "bussi", 512) || !strncmp(thermostat_type, "srd", 512) || !strncmp(thermostat_type, "SRD", 512)) {
		throw oxDNAException("The '%s' thermostat is not yet supported on the ROCM backend", thermostat_type);
	}
