...
CUDASRDThermostat::~CUDASRDThermostat() {
	...
	CUDA_SAFE_CALL(hipFree(_d_cells_dp));
	CUDA_SAFE_CALL(hipFree(_d_reduced_cells_dp));
	CUDA_SAFE_CALL(hipFree(_d_reduce_keys));
	CUDA_SAFE_CALL(hipFree(_d_reduced_cells_keys));
...
void CUDASRDThermostat::init() {
	...
	CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(box_side), &f_copy, sizeof(float)));
	f_copy = this->_r_cell;
	CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(rcell), &f_copy, sizeof(float)));
	f_copy = this->_m;
	CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(m_small), &f_copy, sizeof(float)));
	f_copy = sqrt(this->_m);
	CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(sqrt_m_small), &f_copy, sizeof(float)));
	f_copy = this->_dt * this->_apply_every;
	CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(dt), &f_copy, sizeof(float)));

	CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(N_tot), &_N_tot, sizeof(int)));
	CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(N_solvent), &this->_N_particles, sizeof(int)));
	CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(N_cells_side), &this->_N_cells_side, sizeof(int)));
	CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(max_N_per_cell), &_max_N_per_cell, sizeof(int)));
...
void CUDASRDThermostat::apply_cuda(c_number4 *d_poss, GPU_quat *d_orientations, c_number4 *d_vels, c_number4 *d_Ls, llint curr_step) {
	...
	// reset cells
	CUDA_SAFE_CALL(hipMemset(_d_counters_cells, 0, this->_N_cells * sizeof(int)));
	CUDA_SAFE_CALL(hipMemset(_d_cells_dp, 0, this->_N_cells * _max_N_per_cell * sizeof(c_number4)));

	// copy positions and velocities of the solute particles to the thermostat's arrays
	CUDA_SAFE_CALL(hipMemcpy(_d_poss + this->_N_particles, d_poss, _N_vec_size, hipMemcpyDeviceToDevice));
	CUDA_SAFE_CALL(hipMemcpy(_d_vels + this->_N_particles, d_vels, _N_vec_size, hipMemcpyDeviceToDevice));
	...
	CUDA_SAFE_CALL(hipMemcpy(d_vels, _d_vels + this->_N_particles, _N_vec_size, hipMemcpyDeviceToDevice));
}
