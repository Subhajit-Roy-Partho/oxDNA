...
void CUDABinVerletList::_init_CUDA_verlet_symbols() {
	...
	CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(bverlet_box_side), &f_copy, sizeof(float)));
	CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(bverlet_N), &this->_N, sizeof(int)));
	CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(bverlet_N_cells_side), this->_N_cells_side, 3 * sizeof(int)));
	CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(bverlet_max_N_per_cell), this->_max_N_per_cell, 3 * sizeof(int)));
	CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(bverlet_cells_offset), this->_cells_offset, 3 * sizeof(int)));
	CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(bverlet_counters_offset), this->_counters_offset, 3 * sizeof(int)));
	CUDA_SAFE_CALL(hipMemcpyToSymbol(HIP_SYMBOL(bverlet_AO_mixture), &_AO_mixture, sizeof(bool)));
}
...
void CUDABinVerletList::init(int N, c_number rcut, CUDABox*h_cuda_box, CUDABox*d_cuda_box) {
	...
	CUDA_SAFE_CALL(hipHostMalloc(reinterpret_cast<void **>(&this->_d_cell_overflow), sizeof(bool), hipHostMallocDefault));
...
void CUDABinVerletList::update(c_number4 *poss, c_number4 *list_poss, LR_bonds *bonds) {
	// reset cells
	CUDA_SAFE_CALL(hipMemset(this->_d_counters_cells, 0, _counters_mem));
