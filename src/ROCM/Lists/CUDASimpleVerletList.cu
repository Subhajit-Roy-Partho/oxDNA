...
int CUDASimpleVerletList::_largest_N_in_cells(c_number4 *poss, c_number min_cell_size) {
	...
	int *N_cells_side;
	CUDA_SAFE_CALL(hipHostMalloc(reinterpret_cast<void **>(&N_cells_side), sizeof(int) * 3, hipHostMallocDefault));
	_compute_N_cells_side(N_cells_side, min_cell_size);
	int N_cells = N_cells_side[0] * N_cells_side[1] * N_cells_side[2];

	uint *counters_cells;
	CUDA_SAFE_CALL(hipMalloc(&counters_cells, (size_t ) N_cells * sizeof(uint)));
	CUDA_SAFE_CALL(hipMemset(counters_cells, 0, N_cells * sizeof(uint)));
	...
	std::vector<uint> host_counters(N_cells);
	CUDA_SAFE_CALL(hipMemcpy(host_counters.data(), counters_cells, sizeof(uint) * N_cells, hipMemcpyDeviceToHost));
	int max_N = *std::max_element(host_counters.begin(), host_counters.end());

	CUDA_SAFE_CALL(hipHostFree(N_cells_side));
	CUDA_SAFE_CALL(hipFree(counters_cells));
...
void CUDASimpleVerletList::_init_cells(c_number4 *poss) {
	...
	if(_old_N_cells != -1 && _N_cells != _old_N_cells) {
		...
		hipDestroyTextureObject(_counters_cells_tex);
	...
	if(_d_cells == nullptr) {
		...
		if(poss == nullptr) {
			...
			CUDA_SAFE_CALL(hipMalloc(&poss, (size_t ) N * sizeof(c_number4)));
			CUDA_SAFE_CALL(hipMemcpy(poss, host_positions.data(), sizeof(c_number4) * N, hipMemcpyHostToDevice));
		}
		...
		GpuUtils::init_texture_object(&_counters_cells_tex, hipCreateChannelDesc(32, 0, 0, 0, hipChannelFormatKindSigned), _d_counters_cells, _N_cells);
	}
...
void CUDASimpleVerletList::init(int N, c_number rcut, CUDABox *h_cuda_box, CUDABox *d_cuda_box) {
	...
	CUDA_SAFE_CALL(hipHostMalloc(reinterpret_cast<void **>(&_d_cell_overflow), sizeof(bool), hipHostMallocDefault));
...
std::vector<int> CUDASimpleVerletList::is_large(c_number4 *data) {
	std::vector<c_number4> h_data(_N);
	CUDA_SAFE_CALL(hipMemcpy(h_data.data(), data, sizeof(c_number4) * _N, hipMemcpyDeviceToHost));

	std::vector<int> large_ids;
	for(unsigned int idx = 0; idx < h_data.size(); idx++) {
		if(check_coord_magnitude()(h_data[idx])) {
...
void CUDASimpleVerletList::update(c_number4 *poss, c_number4 *list_poss, LR_bonds *bonds) {
	...
	if(_use_edge) {
		...
		std::vector<int> host_counts(_N + 1);
		CUDA_SAFE_CALL(hipMemcpy(host_counts.data(), _d_number_neighs_no_doubles, sizeof(int) * (_N + 1), hipMemcpyDeviceToHost));
		host_counts[_N] = 0;
		int running = 0;
		for(int i = 0; i < _N + 1; i++) {
			int current = host_counts[i];
			host_counts[i] = running;
			running += current;
		}
		N_edges = host_counts[_N];
		CUDA_SAFE_CALL(hipMemcpy(_d_number_neighs_no_doubles, host_counts.data(), sizeof(int) * (_N + 1), hipMemcpyHostToDevice));
